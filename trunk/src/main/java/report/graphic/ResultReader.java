...
public class ResultReader {
   ...
   private Test readTest(String testLine, boolean isTotals, boolean hasTPS) {
      Scanner scanner = new Scanner(testLine).useDelimiter("\\s{2,}").useLocale(Locale.getDefault());
      ...
      if (scanner.hasNextDouble()) {
         ...
         String finDeLinea = scanner.next().replaceAll("[)]","");
         NumberFormat f = NumberFormat.getInstance(Locale.getDefault());
        
         double firstByteMeanTime = 0.0f;
         
         try {
			firstByteMeanTime = f.parse(finDeLinea).doubleValue();
